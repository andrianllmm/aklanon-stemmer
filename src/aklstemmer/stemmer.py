...
def get_stems(
    ...
    Args:
        ...
        exclude_punc (bool): Whether to exclude punctuation from tokens. Defaults to a list of valid Aklanon words.
...
def get_stem(token: str, valid_words: Optional[list[str]] = valid_words) -> Stem:
    ...
    Args:
        ...
        valid_words (str, optional): A list of valid words to consider for stemming. If None, no validation is performed.
            Defaults to a list of valid Aklanon words.
                ...
                if is_acceptable(stem) and is_vowel(stem[0]):
                    ...
                    if prefix.endswith("ng"):
                        ...
                        if (
                            ...
                            if is_acceptable(stem_ng_rep):
                                # Assimilation (k/null) (e.g. pangangablit => kablit)
                    ...
                    # -m: b/p (e.g. pamahaw => bahaw, pamasyar => pasyar)
                        ...
                        if (
                            ...
                            # Assimilation (b/p)
                            # (e.g. pamamahaw => bahaw, pamamasyar => pasyar)
                            for l in "bp":
                    ...
                    # -n: d/s/t (e.g. panumdum => dumdum, panigarilyo => sigarilyo, panahi => tahi)
                        ...
                        if (
                            ...
                            # Assimilation (d/s/t)
                            # (e.g. panunumdum => dumdum, paninigarilyo => sigarilyo, pananahi => tahi)
                            for l in "dst":
    ...
    for token in tokens:
        for infix in INFIXES:
            if len(token) > len(infix) + 1:
                ...
                if token[0] == "l" and infix == "in":
                    stem_phch_el = "e" + token[3:]
                    stem_phch_el.phoneme_change = "pre: e/l"
                    stems.add(stem_phch_el)

                # <infix>V... (e.g. inabot => abot)
                if token.startswith(infix) and is_vowel(token[2]):
                    stem = token[2:]

                # C<infix>V... (e.g. sinueat => sueat)
                elif (
                    len(token) > 3
                    and token[1:3] == infix
    ...
    for token in tokens:
        for suffix in SUFFIXES:
            if token.endswith(suffix) and len(token) > len(suffix):
                ...
                if suffix in ("ng", "g", "'t", "'y", "t"):
                    # 'g' contraction
                    if suffix == "g" and stem[-1] != "n":
                        continue

                    # "'t" or "'y" contraction
                    if suffix in ("'t", "'y", "t") and not is_vowel(stem[-1]):
                        continue
                ...
                # Phoneme change (n/null) (e.g. eot => eon)
                if len(stem) > 1 and suffix == "t":
                    stem_phch_nnull = stem + "n"
                    stem_phch_nnull.phoneme_change = "suf: n/null"
                    stems.add(stem_phch_nnull)

                # Phoneme change (d/r) (e.g. bayari => bayad)
                if len(stem) > 1 and stem[-1] == "r":
                    stem_phch_dr = replace_letter(stem, -1, "d")
                    stem_phch_dr.phoneme_change = "suf: d/r"
                    stems.add(stem_phch_dr)

                # Phoneme change (d/l) (e.g. sugilanon => sugid)
                elif len(stem) > 1 and stem[-1] == "l":
                    stem_phch_dl = replace_letter(stem, -1, "d")
                    stem_phch_dl.phoneme_change = "suf: d/l"
                    stems.add(stem_phch_dl)

                # Phoneme change (i/y) (e.g. agyan => agi)
                elif len(stem) > 1 and stem[-1] == "y":
                    stem_phch_iy = replace_letter(stem, -1, "i")
                    stem_phch_iy.phoneme_change = "suf: i/y"
                    stems.add(stem_phch_iy)

                # Phoneme change (o/w) (e.g. tubwan => tubo)
                elif len(stem) > 1 and stem[-1] == "w":
                    stem_phch_ow = replace_letter(stem, -1, "o")
                    stem_phch_ow.phoneme_change = "suf: o/w"
                    stems.add(stem_phch_ow)

                # Phoneme change (o/u)
                # (e.g. tauhan => tao)
                elif len(stem) > 1 and stem[-1] == "u":
...
def stem_rep(tokens: set[Stem]) -> set[Stem]:
    ...
    for token in tokens:
        ...
        # Starts with a vowel (V-V) (e.g. aabot => abot)
        if len(token) > 2 and token[0] == token[1] and is_vowel(token[0:2]):
            stem = token[1:]
            stem.rep = str(token[0])

        # Starts with a consonant-vowel (CV-CV) (e.g. babakae => bakae)
            ...
            if len(first) > 1 and len(second) > 1:
                ...
                # 2-character contractions
                    ...
                    if first[:-2] == second:
                        ...
                        # Phoneme change (o/u) (e.g. tawung-tawo)
